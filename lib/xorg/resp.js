...
function button(entry, button, state) {  // {{{2
  ...
  switch (state) {
    ...
    case false:
      ...
      break;
    default:
      M.log.unhandled('Invalid button state', state);
  }
}
...
function key(entry, type, key) {  // {{{2
  switch (type) {
    ...
    case 'keydown':
      ...
      break;
    default:
      M.log.unhandled('Invalid type: ' + type);
      return;
  }
...
function updateMod(entry, name, value) {  // {{{2
  if (typeof value !== 'boolean') {
    M.log.unhandled('Invalid modifier', {name: name, value: value});
    return;
  }

  if (! (name in entry.state.mods)) {
    M.log.unhandled('Unknown modifier', {name: name, value: value});
    return;
  }
