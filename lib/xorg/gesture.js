...
exports.displayLayout = function() {  // {{{2
  ...
  ta.listen('input', onInput.bind(this));
  ta.listen('blur', onBlur.bind(this));
  ta.listen('focus', onFocus.bind(this));
  ta.listen('keyup', onKeyup.bind(this));
...
// Event Handlers {{{1
function tapButton(ma) {  // {{{2
  var name = ma.wrap.attr('label');
  ...
  case 'auto':  // Button is "auto", change it to "on" and send "keydown".
    ...
    break;
  default:
    throw new Error('Invalid button state: ' + ma.wrap.val());
  }
...
function onBlur(ev) {  // {{{2
  ...
  setTimeout(function() {
    if (that.el) {
      that.find('section.touchArea').el.focus();
    }
  });
};
