...
var EntrySocket = M.class('ose/lib/entry/command');
...
exports.post = function(name, data) {  // {{{2
  if (this.xorgClient && this.xorgClient.link && (name in this.xorgClient.link)) {
    ...
    M.log.error(Ose.error(this, 'DISCONNECTED', 'Link is not connected'));
  }
};
...
exports.updateStateKey = function(key, value) {  // {{{2
  ...
  case 'mods':  // Change buttons states depending on incoming mods.
    ...
    break;
  default:
    M.log.unhandled('XORG TOUCH UPDATE STATE Error', value);
  }
};
      ...
      case 2:  // Deletion detected.
        switch (value) {
          ...
          case 'ab':
            ...
            break;
          default:
            M.log.unhandled('Invalid content value', value);
        }
        break;
      case 4:
